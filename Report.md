...
10. Движение осуществляется итерационно по прямой.
11. База имеет фиксированные координаты (x0 = 0, y0 = 0).
13. Геолокация: позиционирование на местности по двум системам - GPS (спутниковая навигационная система) и ИНС (инерциальная навигационная система).
15. Автономность: робот должен корректно продолжать работу в т.ч. при потере связи с сервером или GPS вплоть до накопления некоторой критической ошибки ИНС (для упрощения был взят отрезок в половину расстояния до следующей запланированной точки движения), а иначе попытка возвращения на базу.
17. Х1 и Y1 - адрес доставки заказа.
19. По умолчанию батареи робота хватает на доставку в квадрате с противолежащими вершинами (-100;-100) и (100;100) (т.е. заказы с координатами выходящими за рамки этого квадрата должны отсекаться системой безопасности робота).  
21. Контроль: уровень заряда, распознавание препятствий, поломка. В случае критических проблем дрон должен вернуться на базу.
