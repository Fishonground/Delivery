...
Будут выполнены 7 тестов, 1 общий на корректность работы, 6 на безопасность, ожидаемое время выполнения ~ 6-8 минут.

Ожидаемый результат выглядит следующим образом (часть тестов была удалена или укрупнена, поэтому на скриншоте их больше, чем будет в реальности):

![test](./pics/test_result.png?raw=true "Тестирование")

В настоящий момент часть тестов безопасности не реализованы, поэтому стоит воспользоваться таблицей соответствия тестов сценариям атаки:
...
|gps_broken|см. реализацию|Не может быть реализовано в тесте, т.к. происходит рандомно?|
|password_not_in_destination_point|готов|-|
|bruteforce|готов|-|
|component_lie|см. реализацию|Не может быть реализовано, кроме как вручную, т.к. подразумевается следствием успешной атаки на один из внутренних компонентов|
