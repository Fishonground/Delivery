...
5. Kafka настроена так, что компонены могут читать только свой топик.
...
Будут выполнены 9 тестов, 1 общий на корректность работы, 8 на безопасность, ожидаемое время выполнения зависит от входных (x,y) и в среднем составляет ~ 6-8 минут.
...
|inaction|-|Не может быть реализовано в тесте, т.к. при нормальной функциональности этот механизм не срабатывает, а в тесте подразумевает бездейтсвие -> пришлось бы эмулирвоать бездействие всего компонента. Быстрее при запуске закомментировать одну из функций, например, _requests_queue.put(confirmation_details)_ в строке +- 109 в файле central/consumer.py, после чего в логах системы будет видно сообщение _[confirmation] timeout exeption!_ |
...
|internal_dos|-|Не может быть реализовано в тесте? Т.к. подразумевается некорректная работа компонента. Можно проверить руками, если добавить отправку валидного сообщения невовремя в коде. Будет получена _internal_exeption_ в коде. |
|personal_data_attack|-|Не может быть реализовано в тесте? См. sensitive_output|
|image_appearance|готов|-|
...
|log_losing|TBD|Не может быть реализовано в тесте? Т.к. реализуется заданием прав доступа к файлу и т.д. |
